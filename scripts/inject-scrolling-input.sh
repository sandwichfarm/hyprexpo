...
source_contract() {
    ...
    for token in 'm_events.input.mouse.move.listen' 'm_events.input.mouse.button.listen' 'm_events.input.mouse.axis.listen' 'm_events.input.touch.down.listen' \
        'm_events.input.touch.motion.listen' 'm_events.input.touch.up.listen' 'm_events.input.touch.cancel.listen' 'info.cancelled = effects.consume' 'transitionInput(' 'touchToGlobalLogical('; do
